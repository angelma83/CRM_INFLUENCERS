...
<?php
    ...
    if ($filaListas==0){
      ...
      if ($filas==0){
        ...
        if ($followers>$seguidoresMinimos){
          ...
          echo "El usuario no cumple los requisitos de búsqueda";
        }
      } else{
  ...
  //Conectamos con la base de datos para comprobar si la lista existe o no
  ...
  //Si la lista no existe comenzamos el proceso de búsqueda
  if ($filaListas==0){
    //Comprobamos la red social que estamos utilizando
    if ($redSocial=="Twitter"){
      ...
      foreach($datosTwitter['statuses'] as $tweet){
        ...
        //Limitamos la muestra al número de seguidores indicado en el formulario
        if ($followers>=$seguidoresMinimos){
          echo "<br><br/>";
    ...
    echo "El nombre de la lista ya existe, realiza una nueva búsqueda";
  } 
